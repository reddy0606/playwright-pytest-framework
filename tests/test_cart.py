...
async def test_add_item_to_cart(page):
    ...
    await inventory_page.add_item_to_cart()

    assert await inventory_page.get_cart_count() == "1"


@pytest.mark.asyncio
async def test_remove_item_from_cart(page):
    data = load_test_data()
    ...
    await inventory_page.remove_item_from_cart()

    assert (await page.locator(".shopping_cart_badge").count()) == 0
